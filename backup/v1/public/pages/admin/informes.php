...
if (!$usuario || !in_array((int)$usuario['rol_id'], [ROL_ADMIN, ROL_SUPERADMIN])) {
...
if ($estadoId)    { $where[] = 't.estado_id = ?';         $params[] = $estadoId; }
if ($adminId)     { $where[] = 't.admin_id = ?';          $params[] = $adminId; }
...
// SLA global (tickets cerrados/atendidos histórico) - CON FILTROS
$slaParams = $params;
$st = $db->prepare("SELECT COUNT(*) AS total, SUM(CASE WHEN TIMESTAMPDIFF(HOUR, t.created_at, t.closed_at) <= sc.horas_resolucion THEN 1 ELSE 0 END) AS dentro FROM tickets t JOIN sla_configuracion sc ON sc.prioridad_id = t.prioridad_id AND sc.activo = 1 WHERE $ws AND t.estado_id IN (7,8) AND t.closed_at IS NOT NULL");
...
// ── Chart: Tendencia ──────────────────────────────────────────────────────────
$trendParams = array_merge($params, $params, [$periodo, $periodo]);
$st = $db->prepare(
    "SELECT DATE_FORMAT(fecha, '%Y-%m-%d') AS fecha, SUM(creados) AS creados, SUM(cerrados) AS cerrados
     FROM (
         ...
         FROM tickets t WHERE $ws AND t.created_at >= DATE_SUB(CURDATE(), INTERVAL ? DAY)
         GROUP BY DATE(t.created_at)
         UNION ALL
         SELECT DATE(t.closed_at) AS fecha, 0 AS creados, COUNT(*) AS cerrados
         FROM tickets t WHERE $ws AND t.closed_at IS NOT NULL AND t.closed_at >= DATE_SUB(CURDATE(), INTERVAL ? DAY)
...
$st = $db->prepare(
    "SELECT p.nombre AS prioridad, COUNT(t.id) AS total,
     ...
     WHERE $ws AND t.estado_id IN (7,8) AND t.closed_at IS NOT NULL
...
// ── Tabla: Tickets filtrados ──────────────────────────────────────────────────
$st = $db->prepare(
    "SELECT t.id, t.numero_ticket, t.created_at, t.updated_at, t.estado_id,
            e.nombre AS estado, p.nombre AS prioridad, u.nombre AS solicitante,
            a.nombre AS asignado_a, ts.nombre AS tipo_solicitud
     FROM tickets t
     ...
     WHERE $ws ORDER BY t.created_at DESC LIMIT 50"
);
$st->execute($params);
$ticketsFiltered = $st->fetchAll();
...
function estadoColorInf(int $id): string {
    return match($id) {
        1 => '#5B8FD4',  // En espera
        2 => '#1D9E75',  // En proceso
        3 => '#EF9F27',  // Esperando respuesta
        4 => '#639922',  // Contestado
        5 => '#7F77DD',  // Reabierto
        6 => '#D4537E',  // Reprogramado
        7 => '#5DCAA5',  // Atendido
        8 => '#710E1F',  // Cerrado
        9 => '#E24B4A',  // Rechazado
        default => '#888',
...
<head>
    ...
    <title>Informes — Sistema de Tickets</title>
...
<div class="dash-wrapper">
    ...
    <div class="dash-body-inner">
        ...
        <div class="inf-content">
            ...
            <!-- Tabla: Tickets filtrados (si hay filtro activo) -->
            <?php if ($estadoId || $adminId || $prioridadId || $tipoId): ?>
            <div class="inf-table-card">
                <div class="inf-ctitle" style="margin-bottom:14px">
                    <span>Tickets Filtrados</span>
                    <a href="informes.php" class="inf-clear-filter" style="font-size:12px;color:#666;text-decoration:underline;cursor:pointer;">
                        Limpiar filtros
                    </a>
                </div>
                <table class="inf-data-table">
                    <thead>
                        <tr>
                            <th>ID</th>
                            <th>Solicitante</th>
                    ...
                    <tbody>
                        <?php if (!empty($ticketsFiltered)): ?>
                            <?php foreach ($ticketsFiltered as $tkt): ?>
                            <tr>
                                <td style="font-weight:600;font-family:'DM Mono',monospace"><?= htmlspecialchars($tkt['numero_ticket']) ?></td>
                                <td><?= htmlspecialchars($tkt['solicitante'] ?? 'N/A') ?></td>
                                <td>
                                    <span class="estado-badge <?= estadoBadgeInf((int)$tkt['estado_id'] ?? 1) ?>">
                                        <?= htmlspecialchars($tkt['estado'] ?? 'N/A') ?>
                                    </span>
                                </td>
                                <td><?= htmlspecialchars($tkt['prioridad'] ?? 'N/A') ?></td>
                                <td><?= htmlspecialchars($tkt['tipo_solicitud'] ?? 'N/A') ?></td>
                                <td><?= htmlspecialchars($tkt['asignado_a'] ?? 'Sin asignar') ?></td>
                                <td style="font-size:12px;color:#666"><?= date('d/m/Y H:i', strtotime($tkt['created_at'] ?? 'now')) ?></td>
                            </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
                        <tr>
...
(function(){
    ...
    var PRI_COLORS = ['#1e8c4d','#c97a00','#c9310f'];  // Baja, Media, Alta (de main.css)
    var TIPO_COLORS = ['#378add','#827ade','#1d9e75','#ba7517','#92400e','#4a5568'];  // Incidencia, Requerimiento, Trino, Desarrollo, Mantenimiento, Consulta
                ...
                datasets: [
                    {
                        ...
                        borderColor: '#378add',  // de main.css .tipo-incidencia
                    ...
                    },
                    {
                        ...
                        borderColor: '#1a7a4a',  // de main.css .sla-ok
                ...
                datasets: [
                    {
                        ...
                        backgroundColor: '#1a7a4a',  // de main.css .sla-ok
                    ...
                    },
                    {
                        ...
                        backgroundColor: '#c9310f',  // de main.css .sla-vencido
                        borderRadius: { topLeft: 0, topRight: 4, bottomLeft: 0, bottomRight: 4 },
                        borderSkipped: false,
                    }
                ]
        ...
        new Chart(ctxAsu, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    backgroundColor: '#378add',  // de main.css .tipo-incidencia
                    borderRadius: 5,
                    borderSkipped: false,
                }]
            },
