...
<head>
    ...
    <style>
      ...
      .overlay-backdrop{position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:100;backdrop-filter:blur(3px)}
      .overlay-wrapper{position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);z-index:101;width:90%;max-width:820px;max-height:92vh;overflow-y:auto;border-radius:20px}
      .overlay-wrapper .crear-card{margin:0!important;width:100%!important;max-width:100%!important;border-radius:20px}
      .overlay-msg{padding:0 24px;margin-bottom:4px}
      @media(max-width:768px){.overlay-wrapper{width:96%;max-height:95vh;border-radius:14px}.overlay-wrapper .crear-card{border-radius:14px}.overlay-msg{padding:0 18px}}
      @media(max-width:480px){.overlay-wrapper{width:100%;max-height:100vh;border-radius:0 0 20px 20px;top:auto;bottom:0;transform:translateX(-50%)}.overlay-wrapper .crear-card{border-radius:0 0 20px 20px}.overlay-msg{padding:0 14px}}
    </style>
</head>
...
<div id="dashApp">
    ...
    <div v-if="currentView === 'crear'" class="overlay-wrapper" v-cloak>
      <div class="crear-card" :style="crearIsAdmin ? 'max-width:820px' : ''">

        <!-- Header de marca -->
        <div class="crear-modal-header">
          <div class="cmh-icon"><i class="fa-solid fa-ticket"></i></div>
          <div class="cmh-text">
            <h1 class="crear-title">Crear un ticket</h1>
            <p class="crear-sub">Complete los datos para enviar su solicitud de soporte</p>
          </div>
          <a href="javascript:void(0)" @click="showList" class="crear-back" title="Cerrar">
            <i class="fa-solid fa-xmark"></i>
          </a>
        </div>

        <div v-if="crearSuccess" class="overlay-msg" style="margin-bottom:12px">
          <div style="background:#dcfce7;color:#15803d;border:1px solid #bbf7d0;border-radius:8px;padding:.75rem 1rem;font-size:.875rem;display:flex;align-items:center;gap:8px">
            <i class="fa-solid fa-circle-check"></i>
            Ticket creado correctamente.
            <a href="javascript:void(0)" @click="showList" style="color:#15803d;font-weight:600;margin-left:4px">Volver a mis tickets →</a>
          </div>
        </div>
        <div v-if="crearError" class="overlay-msg" style="margin-bottom:12px">
          <div style="background:#fee2e2;color:#b91c1c;border:1px solid #fecaca;border-radius:8px;padding:.75rem 1rem;font-size:.875rem;display:flex;align-items:center;gap:8px">
        ...
        <form v-if="!crearIsAdmin" @submit.prevent="crearSubmit" class="crear-form">
          ...
          <div class="cc-field">
            <label class="cc-label">Archivos adjuntos <span class="req-hint">(opcional)</span></label>
            <label class="upload-zone" :class="{ 'has-files': crearFileNames }">
              ...
              <span class="upload-zone-text">{{ crearFileNames || 'Arrastra archivos o haz clic para seleccionar' }}</span>
              <span class="upload-zone-hint">PDF, imágenes, documentos Office</span>
            </label>
          </div>
          <button type="submit" class="btn-crear" :disabled="crearLoading">
            <i class="fa-solid fa-paper-plane" style="margin-right:6px"></i>
            {{ crearLoading ? 'Enviando...' : 'Enviar solicitud de soporte' }}
          </button>
        </form>
        ...
        <form v-else @submit.prevent="crearSubmit" class="crear-form">
          ...
          <button type="submit" class="btn-crear" :disabled="crearLoading">
            ...
            {{ crearLoading ? 'Guardando...' : 'Guardar y publicar ticket' }}
          </button>
        </form>
      </div>
    </div>
    ...
    <div v-if="currentView === 'detalle'" class="overlay-wrapper" v-cloak>
      ...
      <div v-else class="crear-card" style="max-width:700px">
        ...
        <!-- Header de marca — detalle -->
        <div class="crear-modal-header">
          <div class="cmh-icon"><i class="fa-solid fa-ticket-simple"></i></div>
          <div class="cmh-text">
            <h1 class="crear-title">{{ detalleTicket.numero_ticket }}</h1>
            <p class="crear-sub">Detalle de la solicitud de soporte</p>
          </div>
          <a href="javascript:void(0)" @click="showList" class="crear-back" title="Cerrar">
            <i class="fa-solid fa-xmark"></i>
          </a>
        ...
        <div class="detalle-body">
          ...
          <div class="det-grid">
            ...
            <div class="det-item">
              <label>Celular</label>
              <span>{{ detalleTicket.celular || '—' }}</span>
            </div>
            <div class="det-item">
              <label>Sitio</label>
          ...
          <div style="margin-bottom:1rem">
            <div style="font-size:11px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--text-secondary);margin-bottom:6px">Mensaje del solicitante</div>
          ...
          <template v-if="detalleIsAdmin">
            ...
            <form @submit.prevent="detalleGuardar" class="crear-form" style="padding:0">
              <div class="crear-row-2">
                <div class="cc-field">
              ...
              <div class="cc-field">
                <label class="cc-label">Archivos adjuntos <span class="req-hint">(opcional)</span></label>
                <label class="upload-zone" :class="{ 'has-files': detalleFileNames }">
                  <input type="file" multiple style="display:none" @change="detalleOnFileChange">
                  <i class="fa-solid fa-cloud-arrow-up upload-zone-icon"></i>
                  <span class="upload-zone-text">{{ detalleFileNames || 'Arrastra archivos o haz clic para seleccionar' }}</span>
                  <span class="upload-zone-hint">PDF, imágenes, documentos Office</span>
                </label>
              </div>
          ...
          <template v-else>
            <div style="display:flex;gap:.5rem;flex-wrap:wrap;align-items:center;margin-bottom:.5rem">
              ...
              <span style="font-size:.78rem;color:var(--text-secondary);margin-left:4px">
          ...
          <div class="section-title" style="margin-top:1rem">Historial</div>
          <div class="timeline">
          ...
          <template v-if="detalleIsAdmin">
            ...
            <button @click="detalleEnviarRespuesta" class="btn-crear" style="margin-top:.75rem" :disabled="detalleRespGuardando">
              <i class="fa-solid fa-paper-plane" style="margin-right:6px"></i>
          ...
          <template v-if="!detalleIsAdmin">
            ...
            <label class="upload-zone" :class="{ 'has-files': detalleUserFileNames }">
              <input type="file" multiple style="display:none" @change="detalleOnFileChangeUser">
              <i class="fa-solid fa-cloud-arrow-up upload-zone-icon"></i>
              <span class="upload-zone-text">{{ detalleUserFileNames || 'Arrastra archivos o haz clic para seleccionar' }}</span>
              <span class="upload-zone-hint">PDF, imágenes, documentos Office</span>
            </label>
            <button @click="detalleSubirArchivosUsuario" class="btn-crear" style="margin-top:.75rem" :disabled="detalleUserUploading">
