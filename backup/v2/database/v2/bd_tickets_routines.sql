...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`tabasco`@`%` SQL SECURITY DEFINER */
/*!50001 VIEW `v_promedio_por_estado` AS select `v`.`estado_id` AS `estado_id`,`v`.`estado` AS `estado`,`e`.`es_terminal` AS `es_terminal`,`e`.`permite_reabrir` AS `permite_reabrir`,count(0) AS `total_registros`,round(avg(`v`.`minutos_en_estado`),1) AS `promedio_minutos`,round((avg(`v`.`minutos_en_estado`) / 60.0),2) AS `promedio_horas`,max(`v`.`minutos_en_estado`) AS `max_minutos`,min(`v`.`minutos_en_estado`) AS `min_minutos` from (`v_tiempos_estado_ticket` `v` join `estados` `e` on((`e`.`id` = `v`.`estado_id`))) group by `v`.`estado_id`,`v`.`estado`,`e`.`es_terminal`,`e`.`permite_reabrir` order by `promedio_minutos` desc */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`tabasco`@`%` SQL SECURITY DEFINER */
/*!50001 VIEW `v_sla_cumplimiento` AS select `t`.`id` AS `ticket_id`,`t`.`numero_ticket` AS `numero_ticket`,`t`.`created_at` AS `fecha_creacion`,`t`.`closed_at` AS `fecha_cierre`,`u`.`nombre` AS `usuario`,`adm`.`nombre` AS `admin`,`p`.`nombre` AS `prioridad`,`ta`.`nombre` AS `tipo_asunto`,`a`.`nombre` AS `asunto`,coalesce(`a`.`horas_resolucion`,`sc`.`horas_resolucion`) AS `sla_comprometido_h`,round((timestampdiff(MINUTE,`t`.`created_at`,`t`.`closed_at`) / 60.0),2) AS `horas_reales`,round(((timestampdiff(MINUTE,`t`.`created_at`,`t`.`closed_at`) / 60.0) - coalesce(`a`.`horas_resolucion`,`sc`.`horas_resolucion`)),2) AS `desviacion_h`,(case when (timestampdiff(MINUTE,`t`.`created_at`,`t`.`closed_at`) <= (coalesce(`a`.`horas_resolucion`,`sc`.`horas_resolucion`) * 60)) then 'dentro_sla' else 'fuera_sla' end) AS `resultado_sla`,`s`.`nombre` AS `sitio`,`ln`.`nombre` AS `linea_negocio` from ((((((((`tickets` `t` join `usuarios` `u` on((`u`.`id` = `t`.`usuario_id`))) left join `usuarios` `adm` on((`adm`.`id` = `t`.`admin_id`))) join `prioridades` `p` on((`p`.`id` = `t`.`prioridad_id`))) left join `asuntos` `a` on((`a`.`id` = `t`.`asunto_id`))) left join `tipos_asunto` `ta` on((`ta`.`id` = `a`.`tipo_asunto_id`))) left join `sla_configuracion` `sc` on((`sc`.`prioridad_id` = `t`.`prioridad_id`))) left join `sitios` `s` on((`s`.`id` = `t`.`sitio_id`))) left join `lineas_negocio` `ln` on((`ln`.`id` = `t`.`linea_negocio_id`))) where ((`t`.`estado_id` = 8) and (`t`.`closed_at` is not null) and (coalesce(`a`.`horas_resolucion`,`sc`.`horas_resolucion`) is not null)) order by `t`.`closed_at` desc */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
