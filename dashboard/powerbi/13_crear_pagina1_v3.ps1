...
# FIX: DataModel debe guardarse como Stored (sin compresion ZIP) igual que el original.
# Todos los demas entradas usan Deflate:Optimal.
...
foreach ($e in $zr.Entries) {
    ...
    $ms.Dispose()
    Write-Host ("  {0,-45} {1,8} bytes" -f $e.FullName, $e.Length) -ForegroundColor Gray
}
$zr.Dispose()
...
Write-Host ("`nReport/Layout enc={0}, {1} chars" -f $enc, $layoutText.Length) -ForegroundColor Gray
...
# Construir ZIP respetando metodo original: DataModel=Stored, resto=Deflate
...
try {
    foreach ($t in $entries) {
        ...
        # DataModel original es Stored (sin compresion ZIP) — respetar metodo original
        ...
        $ne = $zout.CreateEntry($t.Item1, $comp)
        $ns = $ne.Open(); $ns.Write($t.Item2, 0, $t.Item2.Length); $ns.Close()
        Write-Host ("  + {0,-45} [{1}]" -f $t.Item1, $(if ($t.Item1 -eq "DataModel") {"Stored"} else {"Deflate"})) -ForegroundColor Gray
    }
...
Write-Host " DataModel: Stored (igual que el original)" -ForegroundColor Green
Write-Host " Pagina 1: 6 KPI cards" -ForegroundColor Green
Write-Host "=============================================" -ForegroundColor Green
Write-Host ""
Write-Host "Si abre sin error, ejecutar con PBI Desktop abierto:" -ForegroundColor Yellow
Write-Host "  .\9_agregar_medidas_amo.ps1  -> agrega _Medidas" -ForegroundColor White
